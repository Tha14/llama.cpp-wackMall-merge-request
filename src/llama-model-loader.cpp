...
const char * llama_ftype_name(llama_ftype ftype) {
    ...
    switch ((enum llama_ftype) (ftype & ~LLAMA_FTYPE_GUESSED)) {
        ...
        case LLAMA_FTYPE_MOSTLY_TQ3_1S:    name = LLAMA_FTYPE_PREFIX "TQ3_1S - 4.00 bpw WHT-rotated"; break;
        case LLAMA_FTYPE_MOSTLY_TQ4_1S:    name = LLAMA_FTYPE_PREFIX "TQ4_1S - 5.00 bpw WHT-rotated"; break;
...
bool llama_model_loader::load_all_data(
    ...
    if (upload_backend) {
        LLAMA_LOG_DEBUG("%s: using async uploads via %s\n", __func__,
    ...
    // expert tier: register the CPU-resident exps tensors and precompute the
    // ground-truth hash of every expert slice (the hot store verifies its GPU
    // copies against these). GPU-resident exps (e.g. an explicit
    // override-tensor) carry no host data and are skipped here; the hot store
    // filters them the same way, so those layers run stock.
    if (llama_expert_preload::get_slots() > 0) {
        for (struct ggml_tensor * t = ggml_get_first_tensor(ctx); t != nullptr; t = ggml_get_next_tensor(ctx, t)) {
            int il = -1;
            if (!llama_expert_preload::is_exps(ggml_get_name(t), il) || !t->data) {
                continue;
            }
